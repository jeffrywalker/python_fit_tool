...
def _build_workout(
    workout_name: str, workout_message: WorkoutMessage, workout_steps: list[WorkoutStepMessage], out_dir: Path = None
) -> None:
    ...
    _write_workout_fit_and_csv(fit_file=fit_file, workout_name=workout_name, out_dir=out_dir)
...
def create_bike_tempo_workout(out_dir: Path):
    ...
    _build_workout(
        workout_name=workout_name, workout_message=workout_message, workout_steps=workout_steps, out_dir=out_dir
    )
...
def create_pool_swim(out_dir: Path):
    ...
    workout_steps.append(
        swim_rest(
            message_index=len(workout_steps), duration_type=WorkoutStepDuration.REPETITION_TIME, duration_time=120.0
        )
    )
    # create the repeat message
    step = WorkoutStepMessage()
    ...
    _build_workout(workout_name=workout_name, workout_message=workout_message, workout_steps=workout_steps)
